					...
					unsigned long *active, u64 last_alloc)
{
	...
	for (int i = 0; i < map->num_stripes; i++) {
		...
		if (zone_info[i].alloc_offset == WP_CONVENTIONAL) {
			...
			stripe_nr = last_alloc / map->io_width;
			stripe_offset = stripe_nr * map->io_width;
			full_stripe_nr = stripe_nr / map->num_stripes;
			stripe_index = stripe_nr % map->num_stripes;

			zone_info[i].alloc_offset = full_stripe_nr * map->io_width;
			if (stripe_index > i)
				zone_info[i].alloc_offset += map->io_width;
...
int btrfs_load_block_group_zone_info(struct btrfs_block_group *cache, bool new)
{
	...
	case BTRFS_BLOCK_GROUP_DUP:
		ret = btrfs_load_block_group_dup(cache, map, zone_info, active, last_alloc);
		break;
	case BTRFS_BLOCK_GROUP_RAID1:
	case BTRFS_BLOCK_GROUP_RAID1C3:
	case BTRFS_BLOCK_GROUP_RAID1C4:
		ret = btrfs_load_block_group_raid1(cache, map, zone_info, active, last_alloc);
		break;
	case BTRFS_BLOCK_GROUP_RAID0:
		ret = btrfs_load_block_group_raid0(cache, map, zone_info, active, last_alloc);
		break;
	case BTRFS_BLOCK_GROUP_RAID10:
		ret = btrfs_load_block_group_raid10(cache, map, zone_info, active, last_alloc);
		break;
